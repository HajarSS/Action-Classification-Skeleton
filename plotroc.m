function [auc, stack_x, stack_y] = plotroc(y,x,params)
...
y(y==0)= -1;
	...
	elseif exist('params') && ~ischar(params)
		...
		[auc, stack_x, stack_y] = roc_curve(deci*model.Label(1),y);
	else
		if ~exist('params')
			params = [];
		end
		...
		if fold <= 1
			...
			[auc, stack_x, stack_y] = roc_curve(deci,label_y); % plot ROC curve
		end
	end
end
